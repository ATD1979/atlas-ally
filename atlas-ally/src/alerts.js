...
function buildMessage(event, subscriber) {
  ...
  const countryName = country?.name || event.country_code;
  ...
  return (
    ...
    `_${countryName} emergency: ${country?.emergency?.police || '112'}_\n` +
...
async function dispatchAlerts(event) {
  ...
  // Filter: only active trial or premium users
  const eligible = allSubs.filter(s => {
    if (s.plan === 'premium') return true;
    if (s.plan === 'trial') return isTrialActive(s);
    return false;
  });
  ...
  for (const sub of subs) {
    ...
    const msg = buildMessage(event, sub);
    const ok = await sendWhatsApp(sub.whatsapp, msg);
...
// Reusable send for check-ins and zone alerts
async function sendCheckinAlert(toNumber, body) {
  ...
  try {
    await client.messages.create({ from: process.env.TWILIO_WHATSAPP_FROM, to: 'whatsapp:' + clean, body });
    return true;
  } catch(e) { console.error('Checkin WA failed:', e.message); return false; }
}
