...
// cases before flipping NEWS_LLM_DISAMBIGUATION_ENABLED on in production.
//
// Usage:
//   railway run node scripts/test-n20-classifier.js
//
// `railway run` injects the production env (ANTHROPIC_API_KEY, etc.) without
// exposing it in your shell. The script makes real API calls — ~22 of them at
// roughly $0.00025 each, totalling about half a cent.
//
// Exits 1 on any failure so this is CI-friendly later if wanted.
...
// Curated test cases. Mix of strong-tier (no LLM call), noise-filter-caught
// (no LLM call), and weak-tier (LLM call required).
const KNOWN_GOOD = [
...
const KNOWN_NOISE = [
  ...
  // Weak-only noise — LLM has to catch these (not in noise filter blocklist)
...
// Mirror the news.js cache-write pipeline (inline so the script is self-contained
// and tests the actual function exports rather than re-implementing the policy).
