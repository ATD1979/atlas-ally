// db-factory.js — pure factory for atlas DB instances.
//
// Zero side effects on require: importing this module does NOT open a
// connection or touch the filesystem. Call createDb(path) to get a fully
// initialized { db, ...helpers } bundle.
//
// Used by:
//   - src/db.js (prod singleton wiring)
//   - test/helpers/db.js (in-memory test DB per test)
//
// See HANDOFF_v6_37 "Next 3 projects" #1 for the Jest scaffold context.
...
// ─── Schema + migrations ──────────────────────────────────────────────────────
// Idempotent. Safe to run against an empty :memory: DB or an existing prod DB.
// Order matters: ALTER TABLE migrations run BEFORE CREATE TABLE IF NOT EXISTS
// so legacy DBs get new columns; fresh DBs short-circuit the migration guards
// (sqlite_master returns no rows) and the schema block creates everything.
...
// ─── Helpers ──────────────────────────────────────────────────────────────────
// All prepared statements bind to the passed-in db. Call after initSchema(db).
...
// ─── Factory ──────────────────────────────────────────────────────────────────
// Opens a connection, initializes schema, builds helpers. Returns the full
// { db, ...helpers } bundle. Pass ':memory:' for an isolated test DB.
function createDb(dbPath) {
  // Create parent directory for file-backed DBs (skip for :memory:)
  if (dbPath !== ':memory:') {
...
module.exports = { createDb, initSchema, buildHelpers };
